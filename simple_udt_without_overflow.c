...
int main() {
    ...
    while (1) {
        ...
        ret = ckb_checked_load_cell_by_field(
                buffer, &len, 0, i, CKB_SOURCE_INPUT, CKB_CELL_FIELD_LOCK_HASH);
    ...
    while (1) {
        ...
        ret = ckb_load_cell_data(
                (uint8_t*)&current_amount, &len, 0, i, CKB_SOURCE_GROUP_INPUT);
    ...
    while (1) {
        ...
        ret = ckb_load_cell_data(
                (uint8_t*)&current_amount, &len, 0, i, CKB_SOURCE_GROUP_OUTPUT);
        if (ret == CKB_INDEX_OUT_OF_BOUND) {
            break;
        }
        if (ret != CKB_SUCCESS) {
